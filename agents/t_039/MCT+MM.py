...
from agents.t_039.Minimax import myAgent as MM
import math
import random
import time
...
class myAgent(Agent):
    ...
    def selection(self, parent_state, game_state_child_list, visited_state_dict, maximize = True):
        maxUCB = -float('inf')
        ...
        for state in game_state_child_list:
            ...
            if not maximize:
                UCB_val = (1 - total_score/num_of_visit) + math.sqrt(2* math.log(parent_num_of_visit)/num_of_visit)
            else:
    ...
    def simulation(self, game_state, op_id, player_id):
        ...
        total_score = self.gameRule.calScore(game_state, self.id) - self.gameRule.calScore(game_state, (self.id + 1)%2)
    ...
    def SelectAction(self,actions,game_state):
        ...
        if self.countStep > 15:
            best = self.Minimax.SelectAction(actions, game_state)
            return best
        ...
        while time.time() < TIME_LIMIT:
            ...
            while best_child_state_rep in visited_state_dict:
                ...
                best_child_state = self.selection(parent_state_next, game_state_child_list, visited_state_dict, maximize[i])
